...
use zn_spec::{
    ...
    status_summary, update_progress_markdown, validate_proposal_spec,
    write_spec_validation_report,
};
use zn_types::{
    BrainstormSession, BrainstormVerdict, ExecutionEnvelope, ExecutionOutcome, ExecutionPlan,
    ExecutionReport, HostKind, LoopStage, ProjectManifest, Proposal, ProposalStatus,
    RuntimeEvent, TaskStatus,
};
...
pub fn brainstorm(
    ...
    let mut session = if resume {
        ...
        let goal = goal.ok_or_else(|| anyhow!("goal is required when starting a new brainstorm session"))?;
...
fn brainstorm_host_turn_internal(
    ...
    render_host_brainstorm_status(
        project_root,
        &session,
        Some("Started a host-native Brainstorming session from your latest slash-command input.".to_string()),
    )
}
...
fn continue_host_run_after_brainstorm(
    ...
    let proposal = match load_latest_proposal(project_root)? {
        Some(existing)
            if existing.goal == session.goal && has_bound_spec_contract(project_root, &existing.id)? =>
        {
            existing
        }
    ...
    let execution = execute_proposal(project_root, proposal, &session.goal, host, allow_remote_finish)?;
...
fn render_host_brainstorm_status(
        ...
        && next_brainstorm_question(session).is_none()
    {
        ...
        lines.push(format!(
            ...
            project_root.join(".zero_nine/brainstorm/latest-session.json").display()
        ));
        lines.push(
    ...
    let answered = session.questions.iter().filter(|item| item.answered).count();
    ...
    let mut lines = vec![
        ...
        format!("Progress: {}/{} clarification questions answered", answered, total),
        format!(
            "Session files: {} | {}",
            project_root.join(".zero_nine/brainstorm/latest-session.md").display(),
            project_root.join(".zero_nine/brainstorm/latest-session.json").display()
...
pub fn run_goal(project_root: &Path, goal: &str, host: HostKind, allow_remote_finish: bool) -> Result<String> {
    initialize_project(project_root, host.clone())?;

    if matches!(host, HostKind::ClaudeCode | HostKind::OpenCode) {
        if let Some(session) = load_latest_brainstorm_session(project_root)? {
            if session.host == host && !matches!(session.verdict, BrainstormVerdict::Ready) {
                return brainstorm_host_turn_internal(project_root, goal, host, true, allow_remote_finish);
            }
        }
    }
    ...
    let Some(session) = session else {
        return match host {
            HostKind::ClaudeCode | HostKind::OpenCode => brainstorm_host_turn(project_root, goal, host),
            HostKind::Terminal => Ok(host_brainstorm_pause_message(project_root, goal, host)),
        };
    };

    let proposal = match load_latest_proposal(project_root)? {
        Some(existing) if existing.goal == goal && has_bound_spec_contract(project_root, &existing.id)? => existing,
        _ => create_proposal_from_brainstorm(project_root, &session)?,
    };
...
pub fn resume(project_root: &Path, host: HostKind, allow_remote_finish: bool) -> Result<String> {
    ...
    if let Some(proposal) = load_latest_proposal(project_root)? {
        ...
        if proposal.tasks.iter().any(|task| {
            matches!(task.status, TaskStatus::Pending | TaskStatus::Failed | TaskStatus::Running)
        }) {
                    return execute_proposal(project_root, proposal.clone(), &proposal.goal, host, allow_remote_finish);
...
fn finalize_brainstorm(project_root: &Path, session: &BrainstormSession) -> Result<String> {
    ...
    if matches!(session.verdict, BrainstormVerdict::Ready) {
        let proposal = match load_latest_proposal(project_root)? {
            Some(existing) if existing.goal == session.goal && has_bound_spec_contract(project_root, &existing.id)? => existing,
            _ => create_proposal_from_brainstorm(project_root, session)?,
        };
        Ok(format!(
...
fn execute_proposal(
    ...
    while let Some(schedule) = choose_next_ready_batch(&proposal.tasks, max_retries) {
        ...
        state.stage = if batch_runtimes.iter().any(|runtime| runtime.retry_count > 0) {
            LoopStage::Retrying
        } else {
        ...
        for runtime in &batch_runtimes {
            ...
            let envelope = save_execution_envelope(
                project_root,
                &proposal.id,
                &runtime.task.id,
                &envelope,
        ...
        for output in batch_results {
            ...
            let artifacts_dir = proposal_path.join("artifacts").join(format!("task-{}", task.id));
            ...
            match report.outcome {
                ...
                ExecutionOutcome::RetryableFailure => {
                    if retry_count < max_retries {
                        ...
                        state.stage = LoopStage::Retrying;
                        ...
                        if halt_reason.is_none() {
                            halt_reason = Some(
                                report
                                    .failure_summary
                                    .clone()
                                    .unwrap_or_else(|| format!("Task {} exceeded the retry budget of {}.", task.id, max_retries)),
                            );
                        }
                        halted = true;
                    }
                    ...
                    if halt_reason.is_none() {
                        halt_reason = Some(
                            report
                                .failure_summary
                                .clone()
                                .unwrap_or_else(|| format!("Task {} is blocked and requires manual intervention.", task.id)),
                        );
                    }
                    halted = true;
                }
                    ...
                    if halt_reason.is_none() {
                        halt_reason = Some(
                            report
                                .failure_summary
                                .clone()
                                .unwrap_or_else(|| format!("Task {} escalated to manual intervention.", task.id)),
                        );
                    }
                    halted = true;
                }
            ...
            if let Some(candidate) = propose_candidate(&report) {
                let path = project_root
                    ...
                    .join(format!("{}-{}.json", report.task_id, output.iteration_label));
    ...
    if completed {
        ...
        Ok(format!(
            ...
            halt_reason.unwrap_or_else(|| "Execution requires manual review before continuing.".to_string())
        ))
    }
}
    ...
    thread::scope(|scope| {
        ...
        for runtime in batch_runtimes.iter().cloned() {
            ...
            handles.push((
                runtime,
                scope.spawn(move || {
                    execute_task_attempt(&thread_project_root, runtime_for_thread, allow_remote_finish)
                }),
            ));
        }
...
fn infrastructure_failure_report(
    ...
    let mut details = vec![format!("Execution infrastructure failure: {}", failure_summary)];
...
fn choose_next_ready_batch(tasks: &[zn_types::TaskItem], max_retries: u8) -> Option<ReadyBatch> {
    ...
    let mut runnable = tasks
        .iter()
        .enumerate()
        .filter(|(_, task)| matches!(task.status, TaskStatus::Pending | TaskStatus::Running | TaskStatus::Failed))
        .filter(|(_, task)| {
            task.depends_on
                .iter()
    ...
    for (index, task) in runnable {
        ...
        let needs_worktree = matches!(kind.as_str(), "planning" | "execution" | "implementation" | "finish_branch");
        let is_finish = matches!(kind.as_str(), "finish_branch");

        if estimated_retry >= max_retries && matches!(task.status, TaskStatus::Failed) {
            continue;
        }
    ...
    let blocked = tasks
        ...
        .map(|task| {
            let unresolved = task
                ...
                .cloned()
                .collect::<Vec<_>>();
            let retry_budget_exhausted = matches!(task.status, TaskStatus::Failed) && retry_count_for_task(task) >= max_retries;
            (task, unresolved, retry_budget_exhausted)
        })
        .filter(|(_, unresolved, retry_budget_exhausted)| !unresolved.is_empty() || *retry_budget_exhausted)
        .collect::<Vec<_>>();
...
fn scheduler_parallel_window(tasks: &[zn_types::TaskItem]) -> usize {
    let runnable_count = tasks
        ...
        .filter(|task| matches!(task.status, TaskStatus::Pending | TaskStatus::Running | TaskStatus::Failed))
        .count();
    if runnable_count >= 3 { 2 } else { 1 }
}

fn scheduler_worktree_slots(tasks: &[zn_types::TaskItem]) -> usize {
    let has_finish = tasks.iter().any(|task| normalized_task_kind(task) == "finish_branch");
    if has_finish { 1 } else { 2 }
}
...
fn scheduler_priority(task: &zn_types::TaskItem) -> (u8, u8, String) {
    let retry_bias = if matches!(task.status, TaskStatus::Failed | TaskStatus::Running) { 0 } else { 1 };
...
fn save_execution_envelope(
    ...
    if let Some(protocol) = &enriched.context_protocol {
        ...
        fs::write(&proposal_protocol_path, serde_json::to_vec_pretty(protocol)?)?;
...
fn render_task_report(task_title: &str, plan: &ExecutionPlan, report: &ExecutionReport) -> String {
    ...
    if let Some(failure_summary) = &report.failure_summary {
        output.push_str(&format!("- blocking_or_failure_reason: {}\n", failure_summary));
    }
    output.push_str("\n## Plan\n\n");
    ...
    if !report.agent_runs.is_empty() {
        ...
        for run in &report.agent_runs {
            ...
            if let Some(evidence_archive_path) = &run.evidence_archive_path {
                output.push_str(&format!("  - evidence_archive: {}\n", evidence_archive_path));
            }
            output.push_str(&format!(
                "  - replay_ready: {}\n",
...
fn render_proposal_verification(goal: &str, proposal_id: &str, tasks: &[zn_types::TaskItem]) -> String {
    let completed = tasks
        .iter()
        .filter(|task| matches!(task.status, TaskStatus::Completed))
        .count();
