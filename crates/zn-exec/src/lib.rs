...
pub fn start_brainstorm(goal: &str, host: HostKind) -> BrainstormSession {
    ...
    let questions = vec![
        ClarificationQuestion {
            ...
            question: "What exact problem should Zero_Nine solve first, and what outcome would count as success?".to_string(),
            rationale: "A precise problem statement prevents the later spec and execution layers from optimizing for the wrong thing.".to_string(),
            priority: 100,
            answered: false,
        },
        ClarificationQuestion {
            id: "scope_in".to_string(),
            question: "List the capabilities that must be included in the first implementation slice. One item per line is best.".to_string(),
            rationale: "This determines what the first OpenSpec contract must explicitly cover.".to_string(),
            priority: 95,
            answered: false,
        },
        ClarificationQuestion {
            id: "scope_out".to_string(),
            question: "What should explicitly stay out of scope for the first slice?".to_string(),
            rationale: "Explicit exclusions keep the one-command promise realistic and prevent premature expansion.".to_string(),
            priority: 90,
            answered: false,
        },
        ClarificationQuestion {
            id: "constraints".to_string(),
            question: "What non-negotiable constraints must the implementation obey, such as runtime, host, language, workflow, or safety constraints?".to_string(),
            rationale: "Constraints directly affect architecture, workspace strategy, and execution gating.".to_string(),
            priority: 85,
            answered: false,
        },
        ClarificationQuestion {
            id: "acceptance_criteria".to_string(),
            question: "What acceptance criteria should Zero_Nine satisfy before this iteration can be considered successful?".to_string(),
            rationale: "Acceptance criteria define the finish line for review and verification.".to_string(),
            priority: 80,
            answered: false,
        },
        ClarificationQuestion {
...
fn detect_test_command(project_root: &Path) -> String {
    ...
    "printf 'no recognized test stack; skipped adaptive test execution\n'".to_string()
}
    ...
    match kind {
        TaskKind::Implementation | TaskKind::Verification | TaskKind::FinishBranch => vec![
            ...
            VerificationAction {
                ...
                command: "git diff --stat && git diff --check".to_string(),
                required: true,
